...
def run_full_preprocessing(
    external: Dict[str, pd.DataFrame], csv_maps: Dict[str, pd.DataFrame]
) -> pd.DataFrame:
    """
    ...
    product_master_df = enrich_with_reference_data(
        external["query_1"], external["query_2"]
    )
    product_master_df = standardize_dept_name(product_master_df)

    df_item_map = csv_maps["item"]
    df_dept_map = csv_maps["department"]
    product_master_df = apply_item_and_dept_mappings(
        product_master_df, df_item_map, df_dept_map
    )

    transaction_master_df = merge_transaction_with_product_details(
        transaction_df, product_master_df
    )
...
def enrich_with_reference_data(
    rd5500: pd.DataFrame, rd1800: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    merged_df.drop(
        columns=["data_id", "unit_prc", "pos", "dept_code"],
        inplace=True,
        errors="ignore",
    )
...
def standardize_dept_name(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prefix_to_dept = {
        ...
        "EVENT": "EVENT",
        "F": "FUNCTION",
        "FND": "BURGERS AND SANDWICHES",
        "FP": "FP COFFEE",
        "GB": "GO BIGG",
        "KEF": "KIDS EAT FREE",
        "MI": "BIGGS MERCH ITEMS",
        "MOM": "MOM KNOWS BEST",
        "MP": "MARKETING PROMOS",
        "NES": "NESPRESSO",
        "PARTY": "PARTY PACKS",
    ...
    def extract_alpha_prefix(incode):
        return (
            re.match(r"^[A-Z]+", str(incode)).group()
            if re.match(r"^[A-Z]+", str(incode))
            else None
        )
    ...
    df.loc[
        ((df["dep_code"] == 53) | (df["dep_code"] == 54)) & (df["branch"] == "BRLN"),
        "dept_name_std",
    ] = "NANAYS FAVORITE"
    df.loc[
        (df["dep_code"] == 42) & (df["branch"] == "BRLN"), "dept_name_std"
    ] = "BIGGS LOYALTY"
    df.loc[
        (df["incode"] == "SID12") & (df["branch"] == "BRLN"), "dept_name_std"
    ] = "PASTA DISHES"
...
def apply_item_and_dept_mappings(
    df: pd.DataFrame, df_item_map: pd.DataFrame, df_dept_map: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    df = df.merge(
        df_item_map[
            ["ite_desc", "ite_desc_std", "cat_flag_item", "status", "food_type"]
        ],
        on="ite_desc",
        how="left",
    )
...
def merge_transaction_with_product_details(
    transaction_df: pd.DataFrame, product_master_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    merged_df = df.merge(
        product_master_df, on=["branch", "ite_code", "dep_code"], how="left"
    )
    return merged_df
